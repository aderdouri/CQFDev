...
namespace
{
	...
	void printBinaryOptionPriceFile(const char* fileName, double Asset, double Strike, double InterestRate, double Volatility
		, double Expiration, int NumberAssetStep, double BSPrice, double FDMPrice, double ErrorWithBS, double ErrorWithBSPercentage)
	{
		std::ofstream ofs;
...
TEST(BinaryOptionPricing, ExplicitFiniteDifferenceBackwardSchemeTest1)
{
	...
	for (int Asset = 60; Asset <= 110; Asset += 10)
	{
		for (double Expiration = 0.2; Expiration <= 1.0; Expiration += 0.2)
		{
			...
			std::shared_ptr<ExplicitFiniteDifference> FDM(new ExplicitFiniteDifference(Asset, Strike, Expiration
				, InterestRate, Dividend, Volatility, NumberAssetStep));
			FDM->price();

			double err = BS->getPrice() - FDM->getPrice();
			double BSFormula = (BS->getPrice() > 0.0 ? BS->getPrice() : 1); // to avoid the the divding by zero case
			printBinaryOptionPriceFile("binaryCallFDMPrice.csv", Asset, Strike, InterestRate, Volatility, Expiration
				, NumberAssetStep, BS->getPrice(), FDM->getPrice(), err, std::abs(err / BSFormula));
		}
	}
