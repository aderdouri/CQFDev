...
Run: python -m unittest lmmCalibration.CapATMStrikeTests.testATMStrike
...
def load_lmmData():
    ...
    """
    lmmData = pd.read_excel('lmmData.xlsx', sheetname='marketData', skiprows=0)
    lmmData.columns = ['TenorTi', 'Date', 'DiscountFactor', 'CapVolatility']
    return lmmData
...
def processlmmData():
    ...
    df = pd.merge(df, lmmData, how='outer', on='TenorTi', suffixes=('_1', '_2'))
...
class CapATMStrikeTests(TestCase):
    def testATMStrike(self):
        ...
        to the lmmCalibration sheet in the lmmData.xlsx file
        """
        print('ATM strikes for caps, preliminary computations')
        df = processlmmData()
        LiteLibrary.writeDataFrame(df, 'lmmCalibration')
        print('See column ForwardSwapRate from the lmmCalibration sheet in the lmmData.xlsx file.')
